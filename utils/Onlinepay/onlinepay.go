...
// satart:开始时间
// end: 结束时间
func PackageHour(count, satart, end int) []int {
	var _rt []int
	for i := 1; i <= count; i++ {
		now := 0
		if i%2 == 0 {
			now = php2go.Rand(satart, 12)
		} else if i%3 == 0 {
			now = php2go.Rand(10, 13)
		} else {
			now = php2go.Rand(12, end)
		}
		if now+i <= end && now+i >= satart {
			_rt = append(_rt, now+i)
		} else if now+i >= satart {
			_rt = append(_rt, now)
		}
